...
use App\Models\User;
use Modules\Workspace\Notifications\WorkspaceInvitation as WorkspaceInvitationNotification;
use Flux\Flux;
...
<section class="w-full">
  ...
  <x-pages::settings.layout :heading="__('Workspace settings')" :subheading="__('Manage company profile, users and access rights')" :full-width="true">
    <form wire:submit="updateWorkspace"
      class="mt-5 grid gap-4 xl:h-[calc(100vh-13rem)] xl:grid-cols-[360px_minmax(0,1fr)]">
      <section
        class="flex min-h-[560px] flex-col overflow-hidden rounded-lg border border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-950 xl:h-full">
        <div class="flex items-start gap-3 border-b border-zinc-200 px-4 py-3 dark:border-zinc-800">
          <div
            class="flex size-9 shrink-0 items-center justify-center rounded-lg bg-[color-mix(in_oklab,var(--color-brand-primary)_12%,white)] dark:bg-[color-mix(in_oklab,var(--color-brand-primary)_18%,black)]">
        ...
        <div class="flex flex-1 flex-col gap-4 overflow-y-auto px-4 py-4">
          @if ($this->permissions->canUpdateWorkspace)
            <div class="space-y-4">
              <flux:input wire:model="workspaceName" :label="__('Workspace name')" required
      ...
      <section
          ...
          <div class="flex shrink-0 items-center gap-3 pt-1">
            @if ($this->permissions->canAddMember || $this->permissions->canCreateInvitation)
              <flux:modal.trigger name="workspace-members-modal">
                <button type="button"
                  class="cursor-pointer text-sm font-medium text-[#013763] hover:underline dark:text-[#8dc5ff]">
                  {{ __('Invite') }}
                </button>
              </flux:modal.trigger>
            @endif
        ...
        <div class="flex-1 overflow-y-auto">
          <div class="min-w-[640px]">
            <div
              class="grid grid-cols-[minmax(0,1fr)_12rem_2.5rem] items-center gap-3 border-b border-zinc-200 px-4 py-2 text-xs font-medium uppercase tracking-wide text-zinc-400 dark:border-zinc-800 dark:text-zinc-500">
              <span>{{ __('User') }}</span>
              <span>{{ __('Role') }}</span>
            ...
            @forelse ($members as $member)
              <div
                class="grid grid-cols-[minmax(0,1fr)_12rem_2.5rem] items-center gap-3 border-b border-zinc-100 px-4 py-2.5 last:border-b-0 hover:bg-zinc-50 dark:border-zinc-800 dark:hover:bg-black"
                ...
                <div class="flex min-w-0 items-center gap-3">
                  ...
                  <div class="min-w-0">
                    ...
                    <flux:text class="truncate text-xs text-zinc-500 dark:text-zinc-400">{{ $member['email'] }}
                    </flux:text>
                  </div>
                </div>
                ...
                @if ($member['role'] !== 'owner' && $this->permissions->canUpdateMember)
                  <flux:dropdown position="bottom" align="end">
                    <button type="button"
                      class="inline-flex w-full cursor-pointer items-center justify-between gap-2 rounded px-2 py-1 text-left text-sm font-medium text-[#013763] hover:bg-zinc-50 dark:text-[#8dc5ff] dark:hover:bg-zinc-900"
                ...
                @else
                  <span
                    class="truncate px-2 text-sm font-medium text-zinc-700 dark:text-zinc-200">{{ $member['role_label'] }}</span>
                @endif
                  ...
                  @if ($member['role'] !== 'owner' && $this->permissions->canRemoveMember)
                    <flux:modal.trigger name="remove-member-{{ $member['id'] }}">
                      <flux:tooltip :content="__('Remove member')">
                        <flux:button variant="ghost" size="sm" icon="trash" data-test="member-remove-button" />
                      </flux:tooltip>
                    </flux:modal.trigger>
                  @endif
                </div>
              ...
              @if ($member['role'] !== 'owner' && $this->permissions->canRemoveMember)
                <livewire:workspace::pages.remove-member-modal :workspace="$workspaceModel" :member-id="$member['id']"
                  :member-name="$member['name']" :modal-name="'remove-member-'.$member['id']"
            ...
            <div class="mt-4 space-y-3 border-t border-zinc-200 pt-4 dark:border-zinc-800">
              <div class="flex items-center gap-3">
                ...
                <div>
                  ...
                  <flux:text class="text-sm text-zinc-500 dark:text-zinc-400">
                    {{ __('Invitations that have not been accepted yet') }}</flux:text>
                </div>
              </div>
              ...
              <div class="space-y-3">
                @foreach ($invitations as $invitation)
                  <div
                    class="flex items-center justify-between gap-4 rounded-lg border border-zinc-200 bg-zinc-50 px-4 py-3 dark:border-zinc-800 dark:bg-black"
                    data-test="invitation-row">
                    <div class="flex min-w-0 items-center gap-3">
                      <div
                        class="flex size-10 items-center justify-center rounded-lg bg-[color-mix(in_oklab,var(--color-brand-primary)_12%,white)] dark:bg-[color-mix(in_oklab,var(--color-brand-primary)_18%,black)]">
                        <flux:icon name="envelope" class="size-5" />
                      </div>
                      <div class="min-w-0">
                        <div class="truncate font-medium text-zinc-900 dark:text-zinc-100">{{ $invitation['email'] }}</div>
                        <flux:text class="truncate text-sm text-zinc-500 dark:text-zinc-400">{{ $invitation['role_label'] }}
                        </flux:text>
                      </div>
                    </div>
                    ...
                    @if ($this->permissions->canCancelInvitation)
                      <flux:modal.trigger name="cancel-invitation-{{ $invitation['code'] }}">
                        <flux:tooltip :content="__('Cancel invitation')">
                          <flux:button variant="ghost" size="sm" icon="x-mark" data-test="invitation-cancel-button" />
                        </flux:tooltip>
                      </flux:modal.trigger>
                    @endif
                  </div>
                  ...
                  @if ($this->permissions->canCancelInvitation)
                    <livewire:workspace::pages.cancel-invitation-modal :workspace="$workspaceModel"
                      :invitation-code="$invitation['code']" :invitation-email="$invitation['email']"
    ...
    @if ($this->permissions->canDeleteWorkspace && !$workspaceData['is_personal'])
      <section
        class="mt-4 rounded-lg border border-red-200 bg-red-50 px-4 py-4 text-red-700 dark:border-red-200/10 dark:bg-red-900/20 dark:text-red-100">
        <div class="flex items-start gap-3">
          ...
          <div class="flex-1">
            <div class="flex flex-wrap items-center justify-between gap-4">
              <div>
                ...
                <flux:text class="text-sm text-red-700/80 dark:text-red-100/80">
                  {{ __('Permanently delete your workspace') }}</flux:text>
              </div>
  ...
  @if ($this->permissions->canCreateInvitation || $this->permissions->canAddMember)
    <flux:modal name="workspace-members-modal"
      :show="$errors->has('inviteEmail') || $errors->has('inviteRole') || $errors->has('existingUserId') || $errors->has('existingUserRole')"
      focusable class="max-w-6xl">
      <div class="space-y-6">
        <div class="flex items-start justify-between gap-3">
          ...
          <flux:modal.close>
            <button type="button"
              class="inline-flex size-8 items-center justify-center rounded border border-zinc-200 text-zinc-500 hover:bg-zinc-50 hover:text-zinc-700 dark:border-zinc-700 dark:text-zinc-300 dark:hover:bg-zinc-800"
              data-test="workspace-members-modal-close">
        ...
        <div class="space-y-4">
          @if ($this->permissions->canCreateInvitation)
            <div class="flex items-center justify-between gap-3">
              ...
              <a class="inline-flex items-center gap-1.5 text-sm font-medium text-[#013763] hover:underline dark:text-[#8dc5ff]"
                href="#">
            ...
            <form wire:submit="createInvitation" class="flex flex-col gap-2 sm:flex-row sm:items-end">
              <flux:input wire:model="inviteEmail" type="email" :label="false" :placeholder="__('Enter email address')"
                required />
          ...
          <div class="flex items-center justify-between gap-3">
            <flux:text class="text-sm text-zinc-500 dark:text-zinc-400">
              {{ __('Project role management for this workspace is configured in the roles section.') }}</flux:text>
            <a href="{{ route('roles.index') }}"
              class="text-sm font-medium text-[#013763] hover:underline dark:text-[#8dc5ff]">
              {{ __('Role settings') }}
            </a>
          </div>
        </div>
        ...
        @if ($this->permissions->canAddMember)
          <div class="space-y-3 rounded-lg border border-zinc-200 p-4 dark:border-zinc-800">
            ...
            <form wire:submit="addExistingMember" class="grid gap-3 lg:grid-cols-[minmax(0,1fr)_12rem_9rem]">
              <x-ui.select model="existingUserId" :value="$existingUserId" :label="__('User')"
                :options="$availableSystemUsers" :placeholder="__('Select user')"
                :disabled="count($availableSystemUsers) === 0" required />

              <x-ui.select model="existingUserRole" :value="$existingUserRole" :label="__('Role')"
                :options="collect($availableRoles)->mapWithKeys(fn($role) => [$role['value'] => $role['label']])->all()"
                required />

              <flux:button type="submit" size="sm" variant="primary" :disabled="count($availableSystemUsers) === 0"
        ...
        <div class="space-y-2">
          <flux:input wire:model.live.debounce.250ms="memberSearch" type="text" :label="false"
            :placeholder="__('Search by name, position, department')" />

          <div class="overflow-x-auto rounded-lg border border-zinc-200 dark:border-zinc-800">
            <div
              class="grid min-w-[640px] grid-cols-[2rem_minmax(0,1.7fr)_minmax(0,1fr)_minmax(0,1fr)_12rem_2.5rem] items-center gap-3 border-b border-zinc-200 bg-zinc-50 px-4 py-2 text-xs font-medium uppercase tracking-wide text-zinc-400 dark:border-zinc-800 dark:bg-zinc-900 dark:text-zinc-500">
            ...
            @forelse ($this->filteredMembers as $member)
              <div
                class="grid min-w-[640px] grid-cols-[2rem_minmax(0,1.7fr)_minmax(0,1fr)_minmax(0,1fr)_12rem_2.5rem] items-center gap-3 border-b border-zinc-100 px-4 py-2.5 last:border-b-0 hover:bg-zinc-50 dark:border-zinc-800 dark:hover:bg-black">
                <label class="inline-flex size-4 items-center justify-center">
                  <input type="checkbox" checked disabled
                    class="size-4 rounded border-zinc-300 bg-zinc-50 text-[#013763] dark:border-zinc-700 dark:bg-zinc-900 dark:text-[#8dc5ff]" />
                </label>
                ...
                @if ($member['role'] !== 'owner' && $this->permissions->canUpdateMember)
                  <flux:dropdown position="bottom" align="end">
                    ...
                    <flux:menu>
                      @foreach ($availableRoles as $role)
                        <flux:menu.item as="button" type="button"
                          wire:click="updateMember({{ $member['id'] }}, '{{ $role['value'] }}')" data-test="member-role-option">
                          {{ $role['label'] }}
                        </flux:menu.item>
                      @endforeach
                    </flux:menu>
  ...
  @if ($this->permissions->canDeleteWorkspace && !$workspaceData['is_personal'])
    <livewire:workspace::pages.delete-workspace-modal :workspace="$workspaceModel" />
  @endif
</section>
