...
<flux:dropdown position="bottom" align="start">
    <button type="button" class="group flex w-full items-center rounded-lg p-1 hover:bg-zinc-800/5 dark:hover:bg-white/10" data-test="sidebar-menu-button">
        ...
        <div class="in-data-flux-sidebar-collapsed-desktop:hidden mx-2 grid flex-1 text-start text-sm leading-tight">
            ...
            @if($showWorkspace && auth()->user()->currentWorkspace)
                <span class="truncate text-xs text-zinc-400 dark:text-zinc-500">{{ auth()->user()->currentWorkspace->name }}</span>
            @endif
        </div>
        <flux:icon name="chevrons-up-down" variant="micro" class="in-data-flux-sidebar-collapsed-desktop:hidden ms-auto size-4 text-[#013763]" />
    </button>
