...
<flux:modal name="edit-department" focusable class="max-w-none"
  ...
  <div class="flex h-full flex-col">
    ...
    <form wire:submit="updateDepartment" class="flex h-full min-h-0 flex-col">
      <div style="display: flex; gap: 24px; width: 100%; margin-bottom: 24px;">
        <div style="flex: 1 1 0; min-width: 0;">
          <flux:input wire:model="editing_department_name" :label="__('Department name')"
      ...
      <div>
        <flux:input wire:model.live.debounce.300ms="editing_department_employee_search"
            ...
            @foreach ($editingDepartmentWorkers as $worker)
              <tr>
                ...
                <td class="px-4 py-4 text-zinc-700 dark:text-zinc-300">
                  {{ $worker->departments->first()?->name ?: $worker->staffPosition?->department?->name ?: '—' }}
                </td>
      ...
      <div class="flex items-center justify-between gap-3 pt-4">
        <flux:button variant="danger" type="button" wire:click="deleteDepartment">
