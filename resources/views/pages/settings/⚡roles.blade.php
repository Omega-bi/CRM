...
<section class="w-full">
    ...
    <x-pages::settings.layout
        :heading="__('Roles & permissions')"
        :subheading="__('Create roles and assign precise access rights')"
        :full-width="true"
    >
        <form wire:submit="savePermissions" class="mt-4 grid gap-4 xl:h-[calc(100vh-13rem)] xl:grid-cols-[360px_minmax(0,1fr)]">
            <section class="flex min-h-[560px] min-w-0 flex-col overflow-hidden rounded-md border border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-950 xl:h-full">
                <div class="flex items-center justify-between gap-3 border-b border-zinc-200 px-3.5 py-2.5 dark:border-zinc-700">
                    <div class="min-w-0">
                        <flux:heading size="lg">{{ __('Roles') }}</flux:heading>
                    </div>

                    <flux:modal.trigger name="create-role">
                        <button type="button" wire:click="createNewRole" class="inline-flex cursor-pointer items-center gap-2 text-sm text-[#013763] hover:underline dark:text-[#8dc5ff]" data-test="new-role-button">
                ...
                <div class="min-h-0 flex-1 overflow-y-auto">
                    @forelse ($this->roles as $role)
                        <div
                            ...
                            class="group grid grid-cols-[minmax(0,1fr)_2rem] items-center gap-1 border-b border-l-2 border-b-zinc-200 px-3 py-2 last:border-b-0 dark:border-b-zinc-800 {{ $editing_role_id === $role->id ? 'border-l-[var(--color-brand-primary)] bg-zinc-50 dark:bg-black' : 'border-l-transparent' }}"
                            data-test="role-row"
                        >
                            <button
                                type="button"
                                wire:click="editRole({{ $role->id }})"
                                class="grid min-w-0 cursor-pointer grid-cols-[minmax(0,1fr)_auto] items-center gap-2 text-left text-sm"
                            >
                                <span class="min-w-0">
                                    <span class="block truncate font-medium text-zinc-900 transition-colors group-hover:text-[var(--color-brand-primary)] dark:text-zinc-100">{{ $role->name }}</span>
                                    <span class="block truncate text-xs text-zinc-500 transition-colors group-hover:text-zinc-700 dark:group-hover:text-zinc-300">{{ $role->description ?: __('No description') }}</span>
                                </span>
                                <span class="flex shrink-0 items-center gap-1.5 whitespace-nowrap text-xs text-zinc-500 transition-colors group-hover:text-zinc-700 dark:group-hover:text-zinc-300">
                                    <span>{{ __($role->scope === RoleScope::Workspace ? 'Workspace' : 'Project') }}</span>
                    ...
                    @empty
                        <div class="px-4 py-8 text-sm text-zinc-500">
                            {{ __('No roles yet') }}
                        </div>
                    @endforelse
                </div>
            ...
            <section class="flex min-h-[560px] min-w-0 flex-col overflow-hidden rounded-md border border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-950 xl:h-full">
                ...
                <div class="min-h-0 flex-1 overflow-y-auto p-4">
                    <div class="grid gap-3">
                        @foreach ($this->permissionGroups as $group => $permissions)
                            <details wire:key="permission-group-{{ Str::slug($group) }}" class="group rounded-md border border-zinc-200 dark:border-zinc-800" open>
                                <summary class="flex cursor-pointer list-none items-center justify-between gap-3 px-3 py-2.5 text-sm font-medium text-zinc-900 hover:bg-zinc-50 dark:text-zinc-100 dark:hover:bg-black">
                                    <span>{{ __($group) }}</span>
                                ...
                                <div class="divide-y divide-zinc-200 border-t border-zinc-200 dark:divide-zinc-700 dark:border-zinc-700">
                                    @foreach ($permissions as $permission)
                                        <label wire:key="permission-{{ $permission['code'] }}" class="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-4 px-3 py-2.5 text-sm hover:bg-zinc-50 dark:hover:bg-black">
                                            <span class="min-w-0">
                                                <span class="block truncate font-medium text-zinc-900 dark:text-zinc-100">{{ __($permission['name']) }}</span>
                                            ...
                                            <span class="relative inline-flex h-5 w-9 shrink-0 items-center">
                                                <input
                                                    type="checkbox"
                                                    wire:model="selected_permission_codes"
                                                    value="{{ $permission['code'] }}"
                                                    class="peer sr-only"
                                                >
                                                <span class="absolute inset-0 rounded-full bg-zinc-200 transition peer-checked:bg-[var(--color-brand-primary)] dark:bg-zinc-700"></span>
                                                <span class="absolute left-0.5 size-4 rounded-full bg-white shadow-sm transition peer-checked:translate-x-4"></span>
                                            </span>
                                        </label>
                                    @endforeach
                                </div>
                ...
                <div class="flex justify-end border-t border-zinc-200 px-4 py-3 dark:border-zinc-700">
                    <flux:button type="submit" size="sm" variant="primary" :disabled="! $editing_role_id" data-test="save-role-button">
                        {{ __('Save permissions') }}
                    </flux:button>
                </div>
            </section>
        ...
        <flux:modal name="create-role" :show="$role_form_context === 'create' && ($errors->has('role_name') || $errors->has('role_code') || $errors->has('role_scope') || $errors->has('role_description'))" focusable class="max-w-xl">
            <form wire:submit="createRole" class="space-y-5">
                ...
                <div class="grid gap-4">
                    <flux:input wire:model.live="role_name" :label="__('Role name')" required autofocus data-test="role-name-input" />
        ...
        <flux:modal name="edit-role" :show="$role_form_context === 'edit' && ($errors->has('editing_role_id') || $errors->has('role_name') || $errors->has('role_code') || $errors->has('role_scope') || $errors->has('role_description'))" focusable class="max-w-xl">
            <form wire:submit="updateRole" class="space-y-5">
                ...
                <div class="grid gap-4">
                    ...
                    <x-ui.select
                        ...
                        required
                        data-test="edit-role-scope-select"
                    />
                </div>
