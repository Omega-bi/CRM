...
new class extends Component {
    ...
    {
        if (session()->pull('workspace-invitation-accepted')) {
            Flux::toast(variant: 'success', text: __('Invitation accepted.'));
        }
    }

    /**
     * @return \Illuminate\Support\Collection<int, array{code: string, inviter_name: string, workspace_name: string}>
     */
    #[Computed]
    ...
    private function findPendingInvitation(string $code): WorkspaceInvitation
    {
    ...
    @if ($this->pendingInvitations->isNotEmpty())
        <flux:modal name="pending-invitations" wire:model="showPendingInvitationsModal" focusable class="max-w-lg">
            <div data-test="pending-invitations-modal" class="space-y-6">
                <div>
                    <flux:heading size="lg">{{ __('Pending workspace invitations') }}</flux:heading>
                    <flux:subheading>{{ __('Accept or decline the workspaces you have been invited to join.') }}</flux:subheading>
                </div>
                ...
                <div class="grid gap-4">
                    @foreach ($this->pendingInvitations as $invitation)
                        <div data-test="pending-invitation-row" class="rounded-lg border border-zinc-200 p-4 dark:border-zinc-700">
                            <div class="space-y-1">
                                <p class="font-medium">{{ $invitation['workspace_name'] }}</p>
                                <flux:text class="text-sm text-zinc-500 dark:text-zinc-400">
                                    {{ __(':inviter invited you to join this workspace.', ['inviter' => $invitation['inviter_name']]) }}
                                </flux:text>
                            </div>
